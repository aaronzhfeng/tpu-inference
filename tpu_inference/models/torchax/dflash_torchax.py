...
class DFlashTorchaxWrapper:
    ...
    def load(
        ...
        print(
            f"[DIAG-EMBED] target_model_state type: {type(target_model_state).__name__}",
            flush=True)
        print(
            f"[DIAG-EMBED] has .model attr: {hasattr(target_model_state, 'model')}",
            flush=True)
        embed = getattr(target_model_state.model, "embed_tokens", None)
        if embed is None:
            embed = getattr(target_model_state.model, "embed", None)
        print(
            f"[DIAG-EMBED] embed found: {embed is not None}, type: {type(embed).__name__ if embed else None}",
            flush=True)
        if embed is not None:
            if hasattr(embed, "embedding"):
                w = embed.embedding
                print(f"[DIAG-EMBED] embedding attr type: {type(w).__name__}",
                      flush=True)
                # Unwrap nnx.Param → raw jax.Array
                self.embed_weight_jax = w.value if hasattr(w, "value") else w
            elif hasattr(embed, "weight"):
                w = embed.weight
                print(f"[DIAG-EMBED] weight attr type: {type(w).__name__}",
                      flush=True)
                if hasattr(w, "value"):
        ...
        try:
            shape = tuple(self.embed_weight_jax.shape)
            arr_type = type(self.embed_weight_jax).__name__
            print(
                f"[DIAG-EMBED] embed_weight_jax shape={shape} type={arr_type}",
                flush=True)
            import jax.numpy as _jnp
            checksum = float(_jnp.sum(_jnp.abs(self.embed_weight_jax)))
            print(
                f"[DIAG-EMBED] embed_weight_jax abs-sum checksum={checksum:.4f}",
                flush=True)
        except Exception as e:
            print(f"[DIAG-EMBED] introspection failed: {e}", flush=True)
